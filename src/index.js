...
var _padLength = Object.keys(logLevelsByName).map(function (item) { return item.length; }).sort().pop();
...
function createLogger(namespace) {
    ...
    logger.setLevel = function (levelName) {
        ...
        if (!(levelName in logLevelsByName)) {
            throw new Error('Invalid logLevel ' + levelName + ', can be one of: ' + Object.keys(logLevelsByName).join(', '));
        }
        ...
        debugFn.log = function levelLog() {
            if (isNode) {
                ...
                if (arguments[0].substring(0, namespace.length) === namespace) {
                    ...
                    arguments[0] = arguments[0].substr(0, idx) +
                        ' ' +
...
createLogger.enable = function (namespaces) {
    return debug.enable(namespaces instanceof Array ? namespaces.join(',') : namespaces);
}
